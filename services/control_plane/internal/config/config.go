...
type Settings struct {
	...
	EligibilityPolicyPath     string
	EligibilitySigningKeyID   string
	EligibilityKMSKeyVersion  string
	EligibilityEvaluatorEmail string
...
func Schema(serviceName string) []foundationconfig.Field {
	...
	return []foundationconfig.Field{
		...
		{Key: "eligibility.policy_path"},
		{Key: "eligibility.signing_key_id", Default: foundationconfig.String("production-eligibility")},
		{Key: "eligibility.kms_key_version"},
		{Key: "eligibility.evaluator_email"},
...
func EnvironmentSource() foundationconfig.EnvSource {
	return foundationconfig.EnvSource{SourceName: "environment", Mapping: map[string]string{
		...
		"eligibility.policy_path":     "MINDCLADE_ELIGIBILITY_POLICY_PATH",
		"eligibility.signing_key_id":  "MINDCLADE_ELIGIBILITY_SIGNING_KEY_ID",
		"eligibility.kms_key_version": "MINDCLADE_ELIGIBILITY_KMS_KEY_VERSION",
		"eligibility.evaluator_email": "MINDCLADE_ELIGIBILITY_EVALUATOR_EMAIL",
...
func Decode(snapshot foundationconfig.Snapshot) (Settings, error) {
	...
	settings := Settings{
		...
		EligibilityPolicyPath:     value(snapshot, "eligibility.policy_path"),
		EligibilitySigningKeyID:   snapshot.MustGet("eligibility.signing_key_id"),
		EligibilityKMSKeyVersion:  value(snapshot, "eligibility.kms_key_version"),
		EligibilityEvaluatorEmail: value(snapshot, "eligibility.evaluator_email"),
