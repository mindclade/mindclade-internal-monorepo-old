...
func (store *Store) emitReservation(ctx context.Context, action string, record scheduling.Reservation) error {
	...
	return store.emit(ctx, action, ReservationTargetType, record.ID, record.Placement.RunID,
...
func (store *Store) emitQuota(ctx context.Context, domain scheduling.CapacityDomain, nominal scheduling.Demand, epoch uint64) error {
	...
	// operations.
	return store.emit(ctx, "scheduling.quota.put", QuotaTargetType, identifiers.ID{}, class,
...
func (store *Store) emitWeight(ctx context.Context, tenant string, weight uint32, epoch uint64) error {
	...
	return store.emit(ctx, "scheduling.share_weight.put", WeightTargetType, identifiers.ID{}, tenant,
