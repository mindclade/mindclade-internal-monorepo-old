...
//! Process-level conformance for the composed node-agent binary.
//!
...
use mindclade_node_agent::NodeAgentConfig;
...
use mindclade_runtime_core::{ResourceKind, ResourceVector, ResourceVersion};
...
fn temporary_root(label: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "mindclade-node-agent-{label}-{}-{:?}",
        std::process::id(),
        std::time::Instant::now(),
    ));
    std::fs::create_dir_all(&root).expect("temporary artifact root");
    root
}
...
fn config(root: PathBuf) -> BootstrapConfig {
    ...
    BootstrapConfig {
        ...
        node_id: String::from("node-composition-test"),
        artifact_store_root: root,
        agent: NodeAgentConfig {
            node_resources,
            maximum_reference_cache_bytes: 1 << 20,
            maximum_tool_output_bytes: 1 << 16,
            maximum_children: 4,
...
async fn readiness_becomes_true_and_the_process_stops_on_termination() {
    ...
    assert!(
        ready.contains("component=node-agent-artifact-store status=healthy"),
        "{ready}"
    );
    ...
    assert!(
        metrics_body.contains("node_agent_accounting_healthy 1"),
        "{metrics_body}"
    );
...
async fn readiness_drops_when_the_object_store_stops_answering() {
    ...
    assert!(
        unready.contains("component=node-agent-artifact-store status=unhealthy"),
        "{unready}"
    );
...
fn configuration_is_required_rather_than_defaulted() {
    // No node-agent variables are set in this process, so every required input
    // is missing. A composition root that defaulted them would come up with an
    // invented resource envelope and enforce budgets against it.
