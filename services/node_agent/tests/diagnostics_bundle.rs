use mindclade_node_agent::{NodeDiagnosticsBundle, DEFAULT_MAXIMUM_DIAGNOSTIC_BYTES};
...
fn diagnostics_are_bounded_and_deterministic() {
    let bundle = bundle();
    let first = bundle.encode(DEFAULT_MAXIMUM_DIAGNOSTIC_BYTES).expect("encode");
    let second = bundle.encode(DEFAULT_MAXIMUM_DIAGNOSTIC_BYTES).expect("encode");
    assert_eq!(first, second);
    assert_eq!(bundle.estimated_bytes().expect("size"), u64::try_from(first.len()).expect("diagnostic length fits u64"));
}

#[test]
fn diagnostics_reject_sensitive_attributes() {
    let mut bundle = bundle();
    bundle.attributes.insert("private_key_id".into(), "forbidden".into());
