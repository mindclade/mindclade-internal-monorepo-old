use mindclade_node_agent::{
    NodeAgentConfig, NodeHealth, ReferenceCache
};
...
fn validates_node_limits_and_drain() {
    let resources = ResourceVector::new().set(ResourceKind::ResidentMemoryBytes, 1<<30).set(ResourceKind::LocalDiskBytes,
    1<<30).set(ResourceKind::OpenFileDescriptors, 1024).set(ResourceKind::CpuThreads, 8);
    let config = NodeAgentConfig {
        node_resources: resources, maximum_reference_cache_bytes: 1<<30, maximum_tool_output_bytes: 1<<20, maximum_children: 8,
...
fn reference_cache_rejects_accounting_overflow() {
    ...
    cache.activate(ReferenceSnapshot {
