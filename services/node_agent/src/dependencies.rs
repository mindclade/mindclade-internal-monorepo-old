...
//! artifact root unmounted, and the orchestrator would keep sending it stages
//! it cannot stage. The probe below is the cheapest question that still has a
...
//! mechanism; `artifact_proxy` carries its own copy for the same reason.
