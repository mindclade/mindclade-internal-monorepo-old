...
impl NodeAgentComponent {
    #[must_use]
    pub fn new(core: Arc<NodeAgentCore>) -> Self { Self { core } }
    #[must_use]
    pub fn core(&self) -> Arc<NodeAgentCore> { self.core.clone() }
}
impl Component for NodeAgentComponent {
    fn name(&self) -> &str { "node-agent-core" }
    fn start(&mut self) -> FaultResult<()> { Ok(()) }
    fn drain(&mut self) -> FaultResult<()> { self.core.drain(); Ok(()) }
    fn stop(&mut self) -> FaultResult<()> { self.core.drain(); self.core.cancel_active("node agent stopping"); Ok(()) }
}
