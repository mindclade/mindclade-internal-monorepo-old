...
//! Bounded operational HTTP plane: liveness, readiness, and node counters.
//!
//! This is the surface an orchestrator actually consumes, and it is the only
//! listener this process opens. It carries no stage traffic: the node agent's
//! ticketed stage path has no wire contract in `protocols/` yet, and inventing
//! one here would be a protocol decision taken inside a service. See the
//! README's "What this process serves" section — the distinction is
//! load-bearing, and readiness is scoped to exactly what is listed there.

use crate::dependencies::ObjectStoreProbe;
use crate::telemetry::NodeMetrics;
use crate::{NodeAgentCore, NodeHealthSnapshot};
use axum::Router;
...
pub const CORE_REPORT: &str = "node-agent-core";
/// Registry key under which the backing object store publishes.
pub const ARTIFACT_STORE_REPORT: &str = "node-agent-artifact-store";
...
impl OperationsState {
    ...
    ///
    /// The accounting latch is why this exists. `NodeHealth` clears
    /// `accounting_healthy` on a stage-counter under/overflow and nothing in
    /// the crate ever restores it, so from that moment the process refuses
    /// every stage for the rest of its life. Left unpublished that state is
    ...
    /// reconciled with the work it was counting.
    fn publish_core_report(&self) -> NodeHealthSnapshot {
        let snapshot = self.core.health_snapshot();
        let (status, message) = if snapshot.accounting_healthy {
            (HealthStatus::Healthy, "stage accounting is consistent")
        } else {
            (
                HealthStatus::Unhealthy,
                "stage accounting latched corrupt; this process can never admit work again and must be restarted",
            )
        };
    ...
    async fn publish_store_report(&self) {
        ...
        let _ = self.reports.set(ARTIFACT_STORE_REPORT, status, message);
    }
...
pub async fn serve(
    ...
    match timeout(OPERATIONS_DRAIN_TIMEOUT, &mut server).await {
        ...
        Err(_) => Err(io::Error::new(
            ...
            "node agent operational plane exceeded its graceful-drain deadline",
        )),
    }
}
...
async fn metrics(State(state): State<OperationsState>) -> (StatusCode, String) {
    ...
    let _ = writeln!(
        body,
        "node_agent_active_stages {}\nnode_agent_accepting {}\nnode_agent_accounting_healthy {}",
