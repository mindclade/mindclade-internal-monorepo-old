...
impl ToolRequest {
    pub fn validate(&self) -> FaultResult<()> {
        ...
        {
            return Err(Fault::invalid_argument("tool arguments/environment exceed limits"));
        }
        Ok(())
    }
}
...
impl ToolRunner {
    ...
    pub fn run(&self, request: &ToolRequest) -> FaultResult<ToolOutput> {
        ...
        let mut child = command
            .spawn()
            .map_err(|error| Fault::new(Code::Unavailable, "failed to start external tool").with_source(error))?;
        let stdout = child.stdout.take().ok_or_else(|| Fault::internal("tool stdout pipe was not created"))?;
        let stderr = child.stderr.take().ok_or_else(|| Fault::internal("tool stderr pipe was not created"))?;
        ...
        let (status, timed_out) = loop {
            let status = {
                let mut children = self.supervisor.children.lock().unwrap_or_else(|p| p.into_inner());
                let child = children.get_mut(&process.pid).ok_or_else(|| Fault::internal("supervised child disappeared"))?;
                child.try_wait().map_err(|error| Fault::new(Code::Unavailable, "failed to poll external tool").with_source(error))?
            };
            if let Some(status) = status { break (Some(status), false); }
        ...
        if !timed_out {
            self.supervisor.children.lock().unwrap_or_else(|p| p.into_inner()).remove(&process.pid);
        }
...
#[derive(Debug)]
struct CapturedPipe { bytes: Vec<u8>, truncated: bool }

fn spawn_pipe_reader<R: Read + Send + 'static>(name: &str, reader: R, maximum: u64) -> FaultResult<JoinHandle<FaultResult<CapturedPipe>>> {
    thread::Builder::new()
        .name(name.to_owned())
        .spawn(move || drain_pipe(reader, maximum))
        .map_err(|error| Fault::internal("failed to spawn tool pipe reader").with_source(error))
}
fn join_pipe_reader(handle: JoinHandle<FaultResult<CapturedPipe>>, stream: &str) -> FaultResult<CapturedPipe> {
    handle.join().map_err(|_| Fault::internal("tool pipe reader panicked").with_context("stream", stream.to_owned()))?
}
fn drain_pipe<R: Read>(mut reader: R, maximum: u64) -> FaultResult<CapturedPipe> {
    let capacity = usize::try_from(maximum.min(16 * 1024 * 1024))
        .map_err(|_| Fault::new(Code::OutOfRange, "tool output retention capacity exceeds usize"))?;
    let mut retained = Vec::with_capacity(capacity);
    let mut truncated = false;
    let mut chunk = [0_u8; 16 * 1024];
    loop {
        let read = reader.read(&mut chunk).map_err(|error| Fault::new(Code::Unavailable, "failed to read tool output").with_source(error))?;
        if read == 0 { break; }
        let retained_bytes = u64::try_from(retained.len())
            .map_err(|_| Fault::new(Code::OutOfRange, "retained tool output length exceeds u64"))?;
        let remaining = maximum.checked_sub(retained_bytes)
        ...
        let keep = if remaining >= read_bytes {
            ...
            usize::try_from(remaining)
                .map_err(|_| Fault::new(Code::OutOfRange, "tool output retention remainder exceeds usize"))?
        };
        retained.extend_from_slice(&chunk[..keep]);
        if keep < read { truncated = true; }
    }
    Ok(CapturedPipe { bytes: retained, truncated })
}
