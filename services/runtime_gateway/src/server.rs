...
use crate::{
    AdmissionLedger, AdmissionPermit, AdmissionRequest, GatewayConfig, GatewayHealth, PolicyCache,
    RouteRequest, bounded_stream, select_route,
};
...
impl GatewayCore {
    pub fn new(
        ...
        let admission = AdmissionLedger::new(
            ...
            config.maximum_active_requests_per_grant,
        )?;
        Ok(Self {
            config,
            policy,
    ...
    pub fn admit(
        ...
        if !self.health.snapshot().accepting {
            return Err(Fault::new(
                Code::Unavailable,
                "runtime gateway is draining or not accepting",
            ));
        }
        envelope
            .admission
        ...
        let output_limit = if envelope.grant.claims.maximum_output_units == 0 {
            ...
            self.config
                .maximum_stream_output_bytes
                .min(envelope.grant.claims.maximum_output_units)
        };
    ...
    #[must_use]
    pub fn health_snapshot(&self) -> crate::GatewayHealthSnapshot {
