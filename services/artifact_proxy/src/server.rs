...
use crate::{
    ProxyConfig, ProxyHealth, ProxyMetrics, RangeRequest, TransferEngine, ValidatedGrant
};
...
pub struct ArtifactProxyCore {
    config: ProxyConfig, transfer: TransferEngine, health: Arc<ProxyHealth>, metrics: ProxyMetrics
}

impl ArtifactProxyCore {
    pub fn new(config: ProxyConfig, transfer: TransferEngine, health: Arc<ProxyHealth>, metrics: ProxyMetrics) -> FaultResult<Self> {
        config.validate()?;
        Ok(Self {
            config, transfer, health, metrics
        })
    }
    fn begin(&self) -> FaultResult<TransferGuard<'_>> {
        ...
        if !self.health.try_begin(self.config.maximum_concurrent_transfers) {
            self.metrics.rejected();
            return if self.health.snapshot().accepting {
                Err(Fault::new(Code::ResourceExhausted, "artifact proxy transfer limit reached"))
            } else {
    ...
    pub fn write(
        ...
        let _guard = self.begin()?;
        let size = u64::try_from(bytes.len()).map_err(|_| Fault::new(Code::OutOfRange, "artifact proxy write size exceeds u64"))?;
        if size > self.config.maximum_write_bytes {
            return Err(Fault::new(Code::ResourceExhausted, "artifact proxy write exceeds service limit"));
        }
        let digest = self
            .transfer
    ...
    #[must_use] pub fn health_snapshot(&self) -> crate::ProxyHealthSnapshot {
