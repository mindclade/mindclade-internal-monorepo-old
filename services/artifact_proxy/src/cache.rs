    ...
    pub fn insert(&self, digest: Digest, bytes: Vec<u8>) -> FaultResult<()> {
        ...
        if self.maximum_bytes != 0 && size > self.maximum_bytes {
            return Err(Fault::new(
                ...
                "artifact exceeds cache capacity",
            ));
        }
        let mut state = self
            .state
        ...
        let sequence = next_sequence(&mut state)?;
        state.bytes = state.bytes.checked_add(size).ok_or_else(|| {
            Fault::new(Code::OutOfRange, "artifact cache byte accounting overflow")
        })?;
            ...
            || (self.maximum_bytes != 0 && state.bytes > self.maximum_bytes)
        {
            ...
            let Some(victim) = victim else {
                return Err(Fault::data_loss(
                    "artifact cache accounting requires eviction but cache is empty",
                ));
            };
            let removed = state
                .entries
...
fn next_sequence(state: &mut State) -> FaultResult<u64> {
    if state.sequence == u64::MAX {
        let mut order: Vec<(Digest, u64)> = state
            .entries
            .iter()
        ...
        for (index, (digest, _)) in order.into_iter().enumerate() {
            let sequence = u64::try_from(index)
                .map_err(|_| {
                    Fault::new(
                        Code::OutOfRange,
                        "artifact cache sequence compaction exceeds u64",
                    )
                })?
                .checked_add(1)
                .ok_or_else(|| {
                    Fault::new(
                        Code::OutOfRange,
                        "artifact cache sequence compaction overflow",
                    )
                })?;
            state
                .entries
                .get_mut(&digest)
                .ok_or_else(|| {
                    Fault::data_loss("artifact cache sequence compaction lost an entry")
                })?
                .sequence = sequence;
        }
