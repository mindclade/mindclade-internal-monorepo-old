...
use crate::async_ipc::{
    self, AsyncControlSession, AsyncControlSessionFactory,
};
use crate::protocol;
use crate::{
    HostAuthority, HostConfig, HostCore, HostHealth, ModelSpec, ProcessSpec,
    StdProcessLauncher,
};
...
use mindclade_worker_protocol::{
    Ed25519VerificationKey, WorkerState, WorkerStatus,
};
use prost::Message;
...
impl BootstrapConfig {
    pub fn from_env() -> FaultResult<Self> {
        ...
        if socket_path.as_os_str().as_encoded_bytes().len() > MAX_SOCKET_PATH_BYTES {
            return Err(Fault::invalid_argument("runtime-host socket path exceeds platform bound"));
        }
        ...
        let minimum_revocation_epoch = parse_positive_u64("MINDCLADE_RUNTIME_MIN_REVOCATION_EPOCH")?;
        ...
        let host = HostConfig {
            ...
            maximum_control_payload_bytes: parse_positive_u64("MINDCLADE_RUNTIME_MAX_CONTROL_BYTES")?,
...
pub async fn run(config: BootstrapConfig) -> FaultResult<()> {
    ...
    let serve_result = async_ipc::serve_unix_sessions(config.socket_path, factory, shutdown_rx).await;
    core.begin_drain();
    signal.abort();
...
impl ControlSession {
    fn handle_message(&mut self, request: Vec<u8>) -> FaultResult<Vec<u8>> {
        let command = wire::WorkerCommand::decode(request.as_slice())
            .map_err(|error| Fault::invalid_argument("worker control protobuf is invalid").with_source(error))?;
            ...
            .ok_or_else(|| Fault::invalid_argument("worker control command is missing"))?
        {
            ...
            wire::worker_command::Command::Heartbeat(heartbeat) => self.heartbeat(heartbeat, now)?,
        };
        let wire_status = protocol::worker_status(&status);
        let mut encoded = Vec::with_capacity(wire_status.encoded_len());
        wire_status
            .encode(&mut encoded)
            .map_err(|error| Fault::new(Code::Internal, "worker status encoding failed").with_source(error))?;
        Ok(encoded)
    }
    ...
    fn start(&mut self, start: wire::StartCommand, now: u64) -> FaultResult<WorkerStatus> {
        ...
        if start.operation.is_empty() || start.operation.len() > 256 || start.operation.trim() != start.operation {
            return Err(Fault::invalid_argument("worker operation is invalid"));
        }
        let ticket = protocol::execution_ticket(
            start
                .ticket
                .ok_or_else(|| Fault::invalid_argument("start command is missing execution ticket"))?,
        )?;
        let inputs = start
            .inputs
            .into_iter()
    ...
    fn heartbeat(&mut self, heartbeat: wire::HeartbeatCommand, now: u64) -> FaultResult<WorkerStatus> {
        if heartbeat.requested_at_unix_millis == 0 || heartbeat.requested_at_unix_millis > now {
            return Err(Fault::invalid_argument("heartbeat request timestamp is invalid"));
        }
...
fn node_resources_from_env() -> FaultResult<ResourceVector> {
    Ok(ResourceVector::new()
        .set(ResourceKind::CpuMillis, parse_positive_u64("MINDCLADE_RUNTIME_NODE_CPU_MILLIS")?)
        .set(
...
fn model_spec_from_env() -> FaultResult<Option<ModelSpec>> {
    ...
    match (digest, executable) {
        ...
        (Some(digest), Some(executable)) => {
            let model_digest = Digest::from_str(&digest)
                .map_err(|error| Fault::invalid_argument("preloaded model digest is invalid").with_source(error))?;
            let spec = ModelSpec {
                model_digest,
                minimum_gpu_memory_bytes: parse_positive_u64("MINDCLADE_RUNTIME_MODEL_GPU_MEMORY_BYTES")?,
                pinned_memory_bytes: parse_optional_u64("MINDCLADE_RUNTIME_MODEL_PINNED_MEMORY_BYTES")?,
                process: ProcessSpec {
...
fn read_message<M: Message + Default>(path: &PathBuf) -> FaultResult<M> {
    ...
    M::decode(bytes.as_slice())
        .map_err(|error| Fault::invalid_argument("runtime policy protobuf is invalid").with_source(error))
}
...
fn required(name: &'static str) -> FaultResult<String> {
    ...
    if value.is_empty() || value != value.trim() || value.len() > 4096 {
        return Err(Fault::invalid_argument("runtime-host environment value is invalid")
            .with_context("variable", name));
    }
    Ok(value)
}
...
fn parse_positive_u64(name: &'static str) -> FaultResult<u64> {
    ...
    if value == 0 {
        return Err(Fault::invalid_argument("runtime-host integer must be positive")
            .with_context("variable", name));
    }
    Ok(value)
}
...
fn parse_optional_u64(name: &'static str) -> FaultResult<u64> {
    match env::var(name) {
        ...
        Err(error) => Err(Fault::invalid_argument("runtime-host environment value is not Unicode")
            .with_context("variable", name)
            .with_source(error)),
    }
}
...
fn absolute_path(name: &'static str) -> FaultResult<PathBuf> {
    ...
    if !path.is_absolute() {
        return Err(Fault::invalid_argument("runtime-host path must be absolute")
            .with_context("variable", name));
    }
    Ok(path)
}
...
fn unix_millis() -> FaultResult<u64> {
    let elapsed = std::time::SystemTime::now()
        ...
        .map_err(|_| Fault::new(Code::FailedPrecondition, "runtime-host clock is before Unix epoch"))?;
    u64::try_from(elapsed.as_millis())
        .map_err(|_| Fault::new(Code::OutOfRange, "runtime-host clock exceeds u64 milliseconds"))
}
