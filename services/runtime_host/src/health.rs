...
pub struct HostHealth {
    accepting: AtomicBool, process_supervisor_ready: AtomicBool, gpu_ready: AtomicBool
}

impl HostHealth {
    #[must_use] pub fn new() -> Self {
        Self {
            accepting: AtomicBool::new(false), process_supervisor_ready: AtomicBool::new(false), gpu_ready: AtomicBool::new(false)
        }
    }
    ...
    #[must_use] pub fn snapshot(&self) -> HostHealthSnapshot {
        HostHealthSnapshot {
            accepting: self.accepting.load(Ordering::Acquire), process_supervisor_ready: self.process_supervisor_ready
            .load(Ordering::Acquire), gpu_ready: self.gpu_ready.load(Ordering::Acquire)
        }
    }
}
