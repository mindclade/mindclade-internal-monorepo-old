...
from services.workers.batch_inference import build_worker, worker_limits
...
def workload(kind: StageKind = StageKind.BATCH_INFERENCE) -> WorkloadEnvelope:
    stage = StageEnvelope(
...
def test_batch_inference_adapter_composes_shared_bounded_runtime() -> None:
    worker = build_worker(
        Engine(),
...
def test_batch_inference_adapter_binds_the_limits_it_is_given() -> None:
...
def test_batch_inference_adapter_refuses_a_foreign_stage_kind() -> None:
...
def test_batch_inference_adapter_refuses_work_once_draining() -> None:
    worker = build_worker(Engine())
