...
from services.workers.evaluation import build_worker, worker_limits
...
def workload(kind: StageKind = StageKind.EVALUATION) -> WorkloadEnvelope:
    stage = StageEnvelope(
...
def test_evaluation_adapter_composes_shared_bounded_runtime() -> None:
    worker = build_worker(
        Engine(),
...
def test_evaluation_adapter_binds_the_limits_it_is_given() -> None:
...
def test_evaluation_adapter_refuses_a_foreign_stage_kind() -> None:
...
def test_evaluation_adapter_refuses_work_once_draining() -> None:
    worker = build_worker(Engine())
