...
def workload(kind: StageKind = StageKind.REFERENCE_BUILD) -> WorkloadEnvelope:
    stage = StageEnvelope(
...
def test_reference_builder_adapter_binds_the_limits_it_is_given() -> None:
...
def test_reference_builder_adapter_refuses_a_foreign_stage_kind() -> None:
...
def test_reference_builder_adapter_refuses_work_once_draining() -> None:
    worker = build_worker(Engine())
