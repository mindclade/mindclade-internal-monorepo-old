...
from services.workers.rollout import build_worker, worker_limits
...
def workload(kind: StageKind = StageKind.ROLLOUT) -> WorkloadEnvelope:
    stage = StageEnvelope(
...
def test_rollout_adapter_composes_shared_bounded_runtime() -> None:
    worker = build_worker(
        Engine(),
...
def test_rollout_adapter_binds_the_limits_it_is_given() -> None:
...
def test_rollout_adapter_refuses_a_foreign_stage_kind() -> None:
...
def test_rollout_adapter_refuses_work_once_draining() -> None:
    worker = build_worker(Engine())
