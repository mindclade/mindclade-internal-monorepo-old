...
Presubmit uses `ci/common/affected.py` to calculate reverse-dependent Bazel test targets. Global build, toolchain, protocol, or architecture-policy changes intentionally expand to `//...`. The selector is conservative: an owned package without a discoverable test target remains a package-pattern test rather than being treated as unaffected.
