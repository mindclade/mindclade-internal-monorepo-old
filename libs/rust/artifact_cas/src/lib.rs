...
impl Default for CasConfig {
    fn default() -> Self {
        Self {
            maximum_blob_bytes: ByteSize::new(1_u64 << 40), maximum_manifest_bytes: ByteSize::new(64 * 1024 * 1024),
...
impl ArtifactCas {
    pub fn new(store: Arc<dyn ObjectStore>, clock: Arc<dyn Clock>, config: CasConfig) -> FaultResult<Self> {
    ...
    fn manifest_path(manifest: &ArtifactManifest) -> FaultResult<ObjectPath> {
        ObjectPath::new(format!("cas/manifests/{}.mcam", manifest.artifact_id)).map_err(|error| Fault::internal("failed to construct CAS manifest path")
        .with_source(error))
    }
    pub fn put_blob(&self, bytes: &[u8]) -> FaultResult<Digest> {
        ...
        if size > self.config.maximum_blob_bytes.get() {
            return Err(Fault::new(Code::ResourceExhausted, "CAS blob exceeds configured limit"));
        }
    ...
    pub fn get_blob(&self, digest: Digest) -> FaultResult<Vec<u8>> {
        let bytes = self.store.get(&blob::blob_path(digest)?, self.config.maximum_blob_bytes)?;
        digest.verify(&bytes)?;
        Ok(bytes)
    }
    ...
    pub fn publish_manifest(&self, manifest: &ArtifactManifest) -> FaultResult<Digest> {
        ...
        for blob in &manifest.blobs {
            let meta = self.store.head(&blob::blob_path(blob.digest)?)?.ok_or_else(|| Fault::new(Code::FailedPrecondition,
            "artifact manifest references a missing CAS blob").with_context("blob", blob.digest.to_string()))?;
            if meta.digest != blob.digest || meta.size.get() != blob.size {
                return Err(Fault::data_loss("artifact blob metadata does not match manifest"));
            }
        }
        ...
        if encoded_size > self.config.maximum_manifest_bytes.get() {
            return Err(Fault::new(Code::ResourceExhausted, "artifact manifest exceeds configured limit"));
        }
        ...
        match self.store.put(&path, &encoded, PutCondition::CreateOnly) {
            ...
            Err(error) if error.code() == Code::AlreadyExists => {
                ...
                if hash_bytes(&existing) != digest {
                    return Err(Fault::new(Code::Conflict, "artifact ID already has a different manifest"));
                }
                Ok(digest)
            }
            Err(error) => Err(error),
    ...
    pub fn load_manifest(&self, artifact_id: &str) -> FaultResult<ArtifactManifest> {
        let artifact_id = artifact_id.parse::<ResourceId>().map_err(|error| Fault::invalid_argument("artifact ID is invalid")
        .with_source(error))?;
        if artifact_id.kind() != "artifact" {
            return Err(Fault::invalid_argument("artifact ID kind is invalid"));
        }
        let path = ObjectPath::new(format!("cas/manifests/{artifact_id}.mcam")).map_err(|error| Fault::invalid_argument(error
        .to_string()))?;
    ...
    pub fn current_unix_millis(&self) -> FaultResult<u64> {
        let duration = self.clock.system_now().duration_since(UNIX_EPOCH)
        .map_err(|error| Fault::new(Code::FailedPrecondition, "CAS clock is before Unix epoch").with_source(error))?;
