...
pub struct WireContext { pub key: String, pub value: String }
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WireFault { pub code: String, pub message: String, pub context: Vec<WireContext>, pub retry_after_millis: Option<u64> }

impl From<&Fault> for WireFault {
    fn from(fault: &Fault) -> Self {
        let context = fault.context().iter().map(|(key, value)| WireContext {
            key: key.to_owned(),
            value: match value {
