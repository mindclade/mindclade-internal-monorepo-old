...
pub fn encode_traceparent(ctx: &TraceContext) -> FaultResult<String> {
    if ctx.trace_id.len()!=32||ctx.span_id.len()!=16||!ctx.trace_id.bytes().all(|b|b.is_ascii_hexdigit())||!ctx.span_id.bytes().all(|b|b.is_ascii_hexdigit()) {
        return Err(Fault::invalid_argument("trace context is invalid"));
    }
    Ok(format!("00-{}-{}-{:02x}", ctx.trace_id.to_ascii_lowercase(), ctx.span_id.to_ascii_lowercase(), u8::from(ctx.sampled)))
}
...
pub fn decode_traceparent(value: &str) -> FaultResult<TraceContext> {
    ...
    Ok(TraceContext {
        trace_id: p[1].to_owned(), span_id: p[2].to_owned(), sampled: p[3]=="01"
    })
}
