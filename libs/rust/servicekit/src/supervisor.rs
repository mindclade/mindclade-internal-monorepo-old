...
impl Supervisor {
    ...
    where
        F: FnOnce(ShutdownToken) -> FaultResult<()> + Send + 'static,
    {
        let name = name.into();
        if name.is_empty()
            || name.len() > 128
            || self.tasks.iter().any(|(current, _)| current == &name)
        {
            return Err(Fault::invalid_argument(
                "supervised task name is invalid or duplicated",
            ));
        }
        ...
        let handle = thread::Builder::new()
            ...
            .map_err(|error| {
                Fault::internal("failed to spawn supervised task").with_source(error)
            })?;
    ...
    pub fn join(mut self) -> Vec<TaskFailure> {
        ...
        for (name, handle) in self.tasks.drain(..) {
            let fault = match handle.join() {
                ...
                Err(_) => Some(Fault::internal("supervised task panicked")),
            };
            if let Some(fault) = fault {
                if let Ok(mut event) =
                    Event::new("service.task_failed", Severity::Error, self.clock.as_ref())
                {
                    event.attributes.insert("task", name.clone());
                    event.attributes.insert("code", fault.code().to_string());
                    let _ = self.sink.emit(&event);
                }
                failures.push(TaskFailure { task: name, fault });
            }
        }
