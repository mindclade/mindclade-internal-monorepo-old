...
impl StreamReader {
    ...
    pub fn accept(&mut self, item: &PrefetchedShard) -> FaultResult<()> {
        ...
        if item.index != expected {
            return Err(Fault::new(Code::Conflict, "prefetched shard is out of order"));
        }
        self.cursor.shard_index = self.cursor.shard_index.checked_add(1)
            .ok_or_else(|| Fault::new(Code::OutOfRange, "stream cursor shard index exhausted"))?;
        self.cursor.record_index = 0;
        self.cursor.byte_offset = 0;
        Ok(())
    }
    #[must_use]
    pub fn plan(&self) -> &StreamPlan { &self.plan }
}
