...
impl StreamPlan {
    pub fn new(dataset: Name, epoch: u64, seed: u64, world_size: u32, rank: u32, shards: Vec<Shard>) -> FaultResult<Self> {
        if world_size == 0 || rank >= world_size || shards.is_empty() || shards.len() > 1_000_000 {
            return Err(Fault::invalid_argument("stream topology or shard count is invalid"));
        }
        ...
        let assigned = order.into_iter().enumerate()
        .filter_map(|(position, index)| (position % world_size_usize == rank_usize).then(|| shards[index].clone()))
        .collect::<Vec<_>>();
        let digest = plan_digest(&dataset, epoch, seed, world_size, rank, &assigned)?;
        Ok(Self {
            dataset, epoch, seed, world_size, rank, shards: assigned, plan_digest: digest
        })
    }
}
...
impl Cursor {
    #[must_use] pub const fn start(plan_digest: Digest) -> Self {
        Self {
            plan_digest, shard_index: 0, record_index: 0, byte_offset: 0
        }
    }
    ...
    pub fn decode(bytes: &[u8]) -> FaultResult<Self> {
        ...
        if decoder.u16()? != CURSOR_SCHEMA {
            return Err(Fault::new(Code::FailedPrecondition, "stream cursor schema is unsupported"));
        }
        let digest = Digest::from_bytes(<[u8; 32]>::try_from(decoder.bytes()?).map_err(|_| Fault::data_loss("stream cursor digest length is invalid"))?);
        let value = Self {
            plan_digest: digest, shard_index: decoder.u32()?, record_index: decoder.u64()?, byte_offset: decoder.u64()?
        };
        decoder.finish()?;
        Ok(value)
    }
    pub fn validate_for(&self, plan: &StreamPlan) -> FaultResult<()> {
        if self.plan_digest == plan.plan_digest && usize::try_from(self.shard_index).is_ok_and(|index| index <= plan
        .shards.len()) {
            Ok(())
        } else {
            Err(Fault::new(Code::FailedPrecondition, "stream cursor does not match plan"))
        }
    }
}
...
impl Prefetcher {
    pub fn start(plan: StreamPlan, store: Arc<dyn ObjectStore>, maximum_shard_bytes: ByteSize, depth: usize,
    retry_policy: Policy, sleeper: Arc<dyn Sleeper + Send + Sync>) -> FaultResult<Self> {
        ...
        let worker = thread::Builder::new().name("mindclade-data-prefetch".to_owned()).spawn(move || {
            for (index, shard) in plan.shards.into_iter().enumerate() {
                ...
                let result = (|| {
                    ...
                    .map_err(|_| Fault::new(Code::OutOfRange, "stream shard size exceeds u64"))?;
        ...
        }).map_err(|error| Fault::internal("failed to spawn prefetch worker").with_source(error))?;
        Ok(Self {
            receiver: Some(receiver), cancelled, worker: Some(worker)
        })
    }
}
...
fn plan_digest(dataset: &Name, epoch: u64, seed: u64, world_size: u32, rank: u32, shards: &[Shard]) -> FaultResult<Digest> {
    let mut encoder = Encoder::new();
    ...
    encoder.u32(rank);
    encoder.u32(u32::try_from(shards.len()).map_err(|_| Fault::new(Code::OutOfRange, "stream shard count exceeds u32"))?);
    for shard in shards {
