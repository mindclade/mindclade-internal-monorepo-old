...
use mindclade_bio_formats::{a3m, fasta, fastq, stockholm};
...
fn fasta_roundtrip() {
    ...
    assert_eq!(
        fasta::parse(&bytes, Limits::default(), ParseMode::Strict).unwrap(),
        r
    );
}

#[test]
fn a3m_preserves_insertions() {
    let r = a3m::parse(b">x\nACdEF-\n", Limits::default(), ParseMode::Strict).unwrap();
...
fn fastq_roundtrip() {
    ...
    assert_eq!(
        fastq::parse(&b, Limits::default(), ParseMode::Strict).unwrap(),
        r
    );
}
