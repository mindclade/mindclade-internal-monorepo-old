...
pub fn parse(bytes: &[u8], limits: Limits) -> FaultResult<Vec<SdfRecord>> {
    ...
    while let Some((_location, line)) = cursor.next_line()? {
        if line == b"$$$$" {
            if current.is_empty() {
                return Err(Fault::invalid_argument("SDF contains an empty record"));
            }
            if output.len() >= limits.maximum_records {
                return Err(Fault::new(
                    Code::ResourceExhausted,
                    "SDF record limit exceeded",
                ));
            }
            let record = SdfRecord {
    ...
    if output.is_empty() {
        return Err(Fault::invalid_argument("SDF contains no records"));
    }
    Ok(output)
}
