...
pub enum Format { Fasta, Fastq, A3m, Stockholm, Pdb, Mmcif, Sdf, Mol }
...
impl FastqRecord {
    pub fn validate(&self) -> FaultResult<()> {
        if self.id.is_empty() || self.sequence.is_empty() || self.sequence.len() != self.quality.len() {
            return Err(Fault::invalid_argument("FASTQ record is incomplete"));
        }
        if self.quality.iter().any(|value| !(33..=126).contains(value)) {
            return Err(Fault::invalid_argument("FASTQ quality byte is outside printable Phred range"));
        }
        Ok(())
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TextDocument { pub format: Format, pub records: Vec<Vec<u8>> }
...
pub(crate) fn parse_header(
    ...
    let text = std::str::from_utf8(bytes)
        .map_err(|error| Fault::invalid_argument(format!("{kind} header is not UTF-8")).with_source(error))?;
    let mut parts = text.splitn(2, char::is_whitespace);
    let id = parts.next().unwrap_or("").trim();
    if id.is_empty() || id.len() > 4096 {
        return Err(Fault::invalid_argument(format!("{kind} record id is empty or too large")));
    }
    let description = parts.next().unwrap_or("").trim();
    if description.len() > 1_048_576 {
        return Err(Fault::invalid_argument(format!("{kind} description is too large")));
    }
    allocation.charge_usize(id.len().checked_add(description.len()).ok_or_else(|| Fault::invalid_argument("sequence header allocation overflow"))?)?;
...
pub(crate) fn push_sequence(
    ...
    for value in line.iter().copied().filter(|value| !value.is_ascii_whitespace()) {
        ...
        *total_tokens = total_tokens.checked_add(1)
        ...
        allocation.charge(1)?;
        target.push(if preserve_case { value } else { value.to_ascii_uppercase() });
    }
    Ok(())
}
