...
fn mcce1_ticket_matches_go_golden() {
    let claims = ExecutionTicketClaims {
        ticket_id: id("ticket_019c0000000070008000000000000001"),
        issuer: "control".into(),
        tenant_id: id("tenant_019c0000000070008000000000000002"),
        ...
        request_id: None,
        attempt: 1,
        fencing_token: FencingToken::new(9).expect("fence"),
        model_bundle: Some(digest('1')),
        engine_bundle: Some(digest('2')),
        resolved_config_digest: digest('3'),
        reference_snapshot: Some(digest('4')),
        artifacts: ArtifactGrant {
            readable_digests: BTreeSet::from([digest('a')]),
            writable_namespaces: BTreeSet::from(["tenant/t1/run/r1".to_owned()]),
            maximum_read_bytes: 1024,
            maximum_write_bytes: 2048,
            allow_range_reads: true,
            allow_multipart_writes: true,
        },
        budget: ExecutionBudget {
            resources: ResourceVector::new()
                .set(ResourceKind::CpuMillis, 2000)
                .set(ResourceKind::ResidentMemoryBytes, 8 << 30)
                .set(ResourceKind::PinnedMemoryBytes, 1 << 30)
                .set(ResourceKind::SharedMemoryBytes, 512 << 20)
                .set(ResourceKind::LocalDiskBytes, 16 << 30)
                .set(ResourceKind::OpenFileDescriptors, 128)
                .set(ResourceKind::ObjectStoreRequests, 16)
                .set(ResourceKind::QueuedRequests, 8)
                .set(ResourceKind::Processes, 2)
                .set(ResourceKind::CpuThreads, 8)
                .set(ResourceKind::GpuMemoryEstimateBytes, 40 << 30)
                .set(ResourceKind::CheckpointStagingBytes, 4 << 30)
                .set(ResourceKind::TelemetrySpoolBytes, 64 << 20),
            maximum_output_bytes: 2 << 30,
        },
        execution_class: "gpu".into(),
        accelerator_capability: "sm90".into(),
        not_before_unix_millis: 1_800_000_000_000,
        deadline_unix_millis: 1_800_000_600_000,
        expires_unix_millis: 1_800_000_300_000,
        policy_epoch: 12,
        route_snapshot_version: 34,
        revocation_epoch: 7,
