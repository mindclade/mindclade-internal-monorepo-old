...
impl FileSegment {
    pub fn create(
        ...
        if owner_process.is_empty() || owner_process.len() > 256 || generation == 0 {
            return Err(Fault::invalid_argument("portable IPC segment metadata is invalid"));
        }
        let length = u64::try_from(bytes.len())
            .map_err(|_| Fault::new(Code::OutOfRange, "portable IPC segment length exceeds u64"))?;
        if length == 0 || length > MAX_SEGMENT_BYTES {
            return Err(Fault::new(Code::ResourceExhausted, "portable IPC segment exceeds byte limit"));
        }
        fs::create_dir_all(directory).map_err(|error| {
            Fault::new(Code::Unavailable, "failed to create portable IPC directory").with_source(error)
        })?;
        ...
        let mut file = OpenOptions::new()
            ...
            .create_new(true)
            .open(&temp_path)
            .map_err(|error| Fault::new(Code::Unavailable, "failed to create portable IPC segment").with_source(error))?;
        if let Err(error) = file.write_all(bytes).and_then(|()| file.sync_all()) {
            let _ = fs::remove_file(&temp_path);
            return Err(Fault::new(Code::Unavailable, "failed to write portable IPC segment").with_source(error));
        }
        drop(file);
        fs::rename(&temp_path, &final_path).map_err(|error| {
            let _ = fs::remove_file(&temp_path);
            Fault::new(Code::Unavailable, "failed to publish portable IPC segment").with_source(error)
        })?;
        ...
        Ok(Self { path: final_path, descriptor })
    }
    #[must_use]
    ...
    fn read_with_limit(&self, maximum_bytes: u64, now_unix_millis: u64) -> FaultResult<Vec<u8>> {
        ...
        if self.descriptor.range.length() > maximum_bytes {
            return Err(Fault::new(Code::ResourceExhausted, "portable IPC segment exceeds read budget"));
        }
        let mut file = OpenOptions::new()
            .read(true)
            .open(&self.path)
            .map_err(|error| Fault::new(Code::Unavailable, "failed to open portable IPC segment").with_source(error))?;
        let capacity = usize::try_from(self.descriptor.range.length())
            .map_err(|_| Fault::new(Code::OutOfRange, "portable IPC segment length exceeds usize"))?;
        let mut bytes = Vec::with_capacity(capacity);
        file.take(self.descriptor.range.length())
            .read_to_end(&mut bytes)
            .map_err(|error| Fault::new(Code::Unavailable, "failed to read portable IPC segment").with_source(error))?;
    ...
    pub fn remove(mut self) -> FaultResult<()> {
        ...
        match fs::remove_file(path) {
            ...
            Err(error) => Err(Fault::new(Code::Unavailable, "failed to remove portable IPC segment").with_source(error)),
        }
    }
}
