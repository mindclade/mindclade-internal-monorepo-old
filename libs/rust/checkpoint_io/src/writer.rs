...
use crate::{CheckpointManifest, CheckpointShard};
...
impl CheckpointWriter {
    ...
    ) -> FaultResult<CheckpointSession> {
        if run_id.kind() != "run" || world_size == 0 {
            return Err(Fault::invalid_argument(
                "checkpoint run ID or world size is invalid",
            ));
        }
        let checkpoint_id =
            ResourceId::generate("checkpoint", self.clock.as_ref()).map_err(|error| {
                Fault::internal("failed to generate checkpoint ID").with_source(error)
            })?;
...
impl CheckpointSession {
    #[must_use]
    pub fn checkpoint_id(&self) -> &ResourceId {
        &self.checkpoint_id
    }
    pub fn write_shard(
        &mut self,
        ...
        if self.committed {
            return Err(Fault::new(
                ...
                "checkpoint session is already committed",
            ));
        }
        let name = name.into();
        ...
        {
            return Err(Fault::invalid_argument(
                "checkpoint shard fields are invalid",
            ));
        }
        if self.shards.iter().any(|shard| shard.name == name) {
            return Err(Fault::new(
                Code::AlreadyExists,
                "checkpoint shard already exists",
            ));
        }
    ...
    pub fn commit(mut self) -> FaultResult<CheckpointManifest> {
        ...
        let manifest_path = ObjectPath::new(format!("{base}/manifest.mckp"))
            .map_err(|error| Fault::internal(error.to_string()))?;
        self.writer
            .store
            .put(&manifest_path, &encoded, PutCondition::CreateOnly)?;
        let marker_path = ObjectPath::new(format!("{base}/COMMITTED"))
            .map_err(|error| Fault::internal(error.to_string()))?;
        self.writer.store.put(
            &marker_path,
