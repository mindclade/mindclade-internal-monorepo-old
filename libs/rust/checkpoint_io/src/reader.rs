...
impl CheckpointReader {
    ...
    pub fn load(&self, checkpoint_id: &str) -> FaultResult<CheckpointManifest> {
        ...
        let bytes = self
            .store
            .get(&manifest_path, ByteSize::new(64 * 1024 * 1024))?;
        let marker = self
            .store
            .get(&marker_path, ByteSize::new(256))
            .map_err(|error| {
        ...
        let marker = std::str::from_utf8(&marker).map_err(|error| {
            Fault::data_loss("checkpoint commit marker is not UTF-8").with_source(error)
        })?;
        if marker.trim() != hash_bytes(&bytes).to_string() {
            return Err(Fault::data_loss(
                "checkpoint commit marker does not match manifest",
            ));
        }
            ...
            match self.cas.get_blob(shard.digest) {
                Ok(bytes) => {
                    ...
                    if size != shard.size {
                        report
                            .failures
                            .push(format!("{}: size mismatch", shard.name));
                        continue;
                    }
                    report.verified_shards =
                        report.verified_shards.checked_add(1).ok_or_else(|| {
                            Fault::new(Code::OutOfRange, "verified shard count overflow")
                        })?;
                    report.verified_bytes = report
                        .verified_bytes
                        .checked_add(shard.size)
                        .ok_or_else(|| {
                            Fault::new(Code::OutOfRange, "verified checkpoint byte count overflow")
                        })?;
                }
                Err(error) => report
                    .failures
                    .push(format!("{}: {}", shard.name, error.code())),
            }
        }
        Ok(report)
    }
