...
impl CheckpointManifest {
    pub fn validate(&self) -> FaultResult<()> {
        if self.checkpoint_id.kind() != "checkpoint" || self.run_id.kind() != "run" {
            return Err(Fault::invalid_argument("checkpoint or run ID kind is invalid"));
        }
        if self.world_size == 0 || self.shards.is_empty() || self.shards.len() > 1_000_000 {
            return Err(Fault::invalid_argument("checkpoint world size or shard count is invalid"));
        }
        let mut names = BTreeSet::new();
        for shard in &self.shards {
            if shard.name.is_empty() || shard.name.len() > 512 || shard.name.contains("..") || shard.name.starts_with('/') {
                return Err(Fault::invalid_argument("checkpoint shard name is invalid"));
            }
            if shard.rank >= self.world_size {
                return Err(Fault::invalid_argument("checkpoint shard rank exceeds world size"));
            }
            if !names.insert(&shard.name) {
                return Err(Fault::new(Code::AlreadyExists, "checkpoint contains duplicate shard names"));
            }
        }
        if self.components.len() > 4096 {
            return Err(Fault::new(Code::ResourceExhausted, "checkpoint component count exceeds limit"));
        }
    ...
    pub fn encode(&self) -> FaultResult<Vec<u8>> {
        ...
        encoder.u32(u32::try_from(self.shards.len()).map_err(|_| Fault::new(Code::OutOfRange, "checkpoint shard count exceeds u32"))?);
        for shard in &self.shards {
        ...
        encoder.u32(u32::try_from(self.components.len()).map_err(|_| Fault::new(Code::OutOfRange, "checkpoint component count exceeds u32"))?);
        ...
        if bytes.len() > MAX_MANIFEST_BYTES {
            return Err(Fault::new(Code::ResourceExhausted, "checkpoint manifest exceeds size limit"));
        }
        Ok(bytes)
    }
    pub fn decode(bytes: &[u8]) -> FaultResult<Self> {
        ...
        if decoder.u16()? != CHECKPOINT_SCHEMA {
            return Err(Fault::new(Code::FailedPrecondition, "checkpoint schema is unsupported"));
        }
        let checkpoint_id = decoder.string()?.parse::<ResourceId>().map_err(|error| Fault::data_loss("checkpoint ID is invalid")
        .with_source(error))?;
        let run_id = decoder.string()?.parse::<ResourceId>().map_err(|error| Fault::data_loss("run ID is invalid")
        .with_source(error))?;
        let step = decoder.u64()?;
        let world_size = decoder.u32()?;
        let plan = <[u8; 32]>::try_from(decoder.bytes()?).map_err(|_| Fault::data_loss("parallel-plan digest length is invalid"))?;
        ...
        for _ in 0..component_count {
            ...
            let digest = Digest::from_bytes(
                <[u8; 32]>::try_from(decoder.bytes()?)
                    .map_err(|_| Fault::data_loss("checkpoint component digest length is invalid"))?,
            );
            if components.insert(name, digest).is_some() {
                return Err(Fault::data_loss("checkpoint components contain duplicate names"));
            }
        }
        decoder.finish()?;
        let manifest = Self {
            checkpoint_id, run_id, step, world_size, parallel_plan: Digest::from_bytes(plan), shards, components
        };
        manifest.validate()?;
        Ok(manifest)
    }
