...
impl ObjectPath {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, ObjectPathError> {
        ...
        if value.is_empty() || value.len() > Self::MAX_LEN || value.contains('\\') || value.contains('\0') {
            return Err(ObjectPathError("object path is empty, too long, or contains unsupported characters"));
        }
        ...
        for component in path.components() {
            match component {
                Component::Normal(_) => {}
                _ => return Err(ObjectPathError("object path contains traversal or platform prefixes")),
            }
        }
