...
impl ObjectStore for MemoryStore {
    ...
    fn get(&self, path: &ObjectPath, maximum_bytes: ByteSize) -> FaultResult<Vec<u8>> {
        let guard = self.entries.read().unwrap_or_else(|poisoned| poisoned.into_inner());
        let entry = guard.get(path).ok_or_else(|| Fault::new(Code::NotFound, "object not found"))?;
        if entry.meta.size.get() > maximum_bytes.get() {
            return Err(Fault::new(Code::ResourceExhausted, "object exceeds read limit"));
        }
    ...
    fn get_range(&self, path: &ObjectPath, range: ByteRange) -> FaultResult<Vec<u8>> {
        let guard = self.entries.read().unwrap_or_else(|poisoned| poisoned.into_inner());
        let entry = guard.get(path).ok_or_else(|| Fault::new(Code::NotFound, "object not found"))?;
        let start = usize::try_from(range.start()).map_err(|_| Fault::new(Code::OutOfRange, "range start exceeds platform limits"))?;
        let end = usize::try_from(range.end()).map_err(|_| Fault::new(Code::OutOfRange, "range end exceeds platform limits"))?;
    ...
    fn put(&self, path: &ObjectPath, bytes: &[u8], condition: PutCondition) -> FaultResult<PutResult> {
        ...
        guard.insert(path.clone(), Entry {
            bytes: bytes.to_vec(), meta: meta.clone()
        });
        Ok(PutResult {
            meta, created
        })
    ...
    fn list(&self, prefix: Option<&ObjectPath>, limit: usize) -> FaultResult<Vec<ObjectMeta>> {
        ...
        let guard = self.entries.read().unwrap_or_else(|poisoned| poisoned.into_inner());
        Ok(guard.values().filter(|entry| prefix.is_none_or(|value| entry.meta.path.as_str().starts_with(value)))
        .take(limit).map(|entry| entry.meta.clone()).collect())
    }
}
