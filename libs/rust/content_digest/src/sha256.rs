...
impl Sha256 {
    ...
    pub fn update(&mut self, mut input: &[u8]) {
        // SHA-256 encodes the message length modulo 2^64 bits. Use explicit
        // wrapping arithmetic rather than saturation: saturation changes the
        // encoded length and is not part of the SHA-256 specification.
        let input_bytes = u64::try_from(input.len()).unwrap_or(u64::MAX);
        self.length_bytes = self.length_bytes.wrapping_add(input_bytes);
    ...
    pub fn finalize(mut self) -> Digest {
        // Per FIPS 180-4, the final block contains the low 64 bits of the
