...
use super::{
    Budget, Reservation, ResourceVector
};
...
pub struct Allocation {
    pub owner: String, reservation: Reservation
}
...
impl Allocation {
    pub fn acquire(budget: &Arc<Budget>, request: AllocationRequest) -> FaultResult<Self> {
        request.validate()?;
        Ok(Self {
            owner: request.owner, reservation: budget.reserve(request.resources)?
        })
    }
    #[must_use] pub fn reservation(&self) -> &Reservation {
        &self.reservation
    }
}
