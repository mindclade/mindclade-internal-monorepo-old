...
impl Policy {
    pub fn validate(self) -> FaultResult<Self> {
        if self.max_attempts == 0 {
            return Err(Fault::invalid_argument("retry max_attempts must be positive"));
        }
        if self.initial_delay > self.maximum_delay {
            return Err(Fault::invalid_argument("retry initial delay exceeds maximum delay"));
        }
        if self.maximum_delay.as_nanos() > u128::from(u64::MAX) {
            return Err(Fault::new(Code::OutOfRange, "retry maximum delay exceeds nanosecond domain"));
        }
        if self.multiplier_milli < 1_000 {
            return Err(Fault::invalid_argument("retry multiplier must be at least 1.0"));
        }
    ...
    pub fn delay(self, attempt: u32, seed: u64) -> FaultResult<Duration> {
        ...
        for _ in 1..attempt {
            let scaled = nanos.checked_mul(u128::from(policy.multiplier_milli))
                .ok_or_else(|| Fault::new(Code::OutOfRange, "retry backoff multiplication overflow"))?;
            nanos = (scaled / 1_000).min(maximum_nanos);
        }
        let random_value = splitmix64(seed ^ u64::from(attempt)) % 20_001;
        let random = i128::from(i32::try_from(random_value)
            .map_err(|_| Fault::new(Code::OutOfRange, "retry random value exceeds i32"))?) - 10_000;
        ...
        let adjustment = base
            ...
            / 100_000_000;
        let adjusted = base.checked_add(adjustment)
