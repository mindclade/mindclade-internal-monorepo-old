...
use mindclade_runtime_core::{Budget, CancellationToken, FencingToken, Precondition, ResourceKind, ResourceVector, ResourceVersion, TaskGroup};
...
fn budgets_are_hierarchical_and_fencing_rejects_stale_work() {
    ...
    let worker=Budget::child(root.clone(), "worker", ResourceVector::new().set(ResourceKind::ResidentMemoryBytes, 80));
    let reservation=worker.reserve(ResourceVector::new().set(ResourceKind::ResidentMemoryBytes, 64));
    assert!(reservation.is_ok());
    assert!(worker.reserve(ResourceVector::new().set(ResourceKind::ResidentMemoryBytes, 32)).is_err());
    let current=FencingToken::new(2).expect("token");
    assert!(FencingToken::new(1).expect("token").require_current(current).is_err());
    drop(reservation);
}
