...
impl TensorManifest {
    pub fn validate(&self) -> FaultResult<()> {
        ...
        if self.dtype.is_empty()||self.shape.is_empty()||self.shape.len()>16||self.shape.contains(&0) {
        ...
        let elements=self.shape.iter().try_fold(1u128, |n, d|n.checked_mul(u128::from(*d)).ok_or_else(||Fault::new(Code::OutOfRange,
