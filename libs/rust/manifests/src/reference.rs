...
impl ArtifactRef {
    pub fn validate(&self) -> FaultResult<()> {
        ...
        if self.logical_kind.is_empty() || self.logical_kind.len()>128 || !self.logical_kind.is_ascii() {
            return Err(Fault::invalid_argument("artifact logical kind is invalid"));
        }
        if self.schema_version==0 {
            return Err(Fault::invalid_argument("artifact schema version must be non-zero"));
        }
        Ok(())
    }
}
