    ...
    pub fn new(
        ...
        if media_type.is_empty() || media_type.len() > 256 || !media_type.is_ascii() {
            return Err(Fault::invalid_argument(
                "artifact blob media type is invalid",
            ));
        }
        Ok(Self {
            path,
...
impl ArtifactManifest {
    pub fn new(artifact_id: ResourceId, kind: Name, blobs: Vec<BlobRef>) -> FaultResult<Self> {
        let logical_size = blobs.iter().try_fold(0_u64, |total, blob| {
            total
                .checked_add(blob.size)
                .ok_or_else(|| Fault::new(Code::OutOfRange, "artifact logical size overflow"))
        })?;
        let manifest = Self {
            artifact_id,
            kind,
    ...
    pub fn insert_metadata(
        ...
        if self.metadata.len() >= MAX_METADATA && !self.metadata.contains_key(&key) {
            return Err(Fault::new(
                ...
                "artifact metadata limit exceeded",
            ));
        }
    ...
    pub fn validate(&self) -> FaultResult<()> {
        ...
        if self.metadata.len() > MAX_METADATA {
            return Err(Fault::invalid_argument(
                "artifact metadata count is invalid",
            ));
        }
        ...
        for blob in &self.blobs {
            if !paths.insert(blob.path.as_str()) {
                return Err(Fault::new(
                    Code::AlreadyExists,
                    "artifact contains duplicate blob paths",
                ));
            }
            total = total
                .checked_add(blob.size)
        ...
        if total != self.logical_size {
            return Err(Fault::data_loss(
                "artifact logical size does not match blob inventory",
            ));
        }
        Ok(())
    }
    pub fn encode(&self) -> FaultResult<Vec<u8>> {
        self.validate()?;
        let estimated_blob_bytes = self.blobs.len().checked_mul(96).ok_or_else(|| {
            Fault::new(
                Code::OutOfRange,
                "artifact manifest capacity calculation overflow",
            )
        })?;
        let estimated_capacity = 256_usize
            .checked_add(estimated_blob_bytes)
            .ok_or_else(|| {
                Fault::new(
                    Code::OutOfRange,
                    "artifact manifest capacity calculation overflow",
        ...
        encoder.u32(
            u32::try_from(self.blobs.len())
                .map_err(|_| Fault::new(Code::OutOfRange, "artifact blob count exceeds u32"))?,
        );
        for blob in &self.blobs {
        ...
        encoder
            .u32(u32::try_from(self.metadata.len()).map_err(|_| {
                Fault::new(Code::OutOfRange, "artifact metadata count exceeds u32")
            })?);
        for (key, value) in &self.metadata {
            encoder.string(key)?;
        ...
        if bytes.len() > MAX_ENCODED_BYTES {
            return Err(Fault::new(
                Code::ResourceExhausted,
                "artifact manifest exceeds encoded size limit",
            ));
        }
        Ok(bytes)
    }
    pub fn decode(bytes: &[u8]) -> FaultResult<Self> {
        ...
        let artifact_id = decoder
            .string()?
            .parse::<ResourceId>()
            .map_err(|error| Fault::data_loss("artifact ID is invalid").with_source(error))?;
        let kind = Name::new(decoder.string()?)
            .map_err(|error| Fault::data_loss("artifact kind is invalid").with_source(error))?;
        ...
        for _ in 0..blob_count {
            let path = Name::new(decoder.string()?).map_err(|error| {
                Fault::data_loss("artifact blob path is invalid").with_source(error)
            })?;
            let digest_bytes = decoder.bytes()?;
            let digest_array = <[u8; 32]>::try_from(digest_bytes)
                .map_err(|_| Fault::data_loss("artifact blob digest length is invalid"))?;
        ...
        for _ in 0..metadata_count {
            ...
            if metadata.insert(key, value).is_some() {
                return Err(Fault::data_loss(
                    "artifact metadata contains duplicate keys",
                ));
            }
        }
        decoder.finish()?;
...
fn validate_metadata_entry(key: &str, value: &str) -> FaultResult<()> {
    ...
    {
        return Err(Fault::invalid_argument(
            "artifact metadata entry is invalid",
        ));
    }
    Ok(())
}
