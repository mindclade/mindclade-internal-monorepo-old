...
impl DeliveryBatch {
    pub fn new(envelopes: Vec<Envelope>, maximum_bytes: u64) -> FaultResult<Self> {
        ...
        for envelope in &envelopes {
            let payload_bytes = u64::try_from(envelope.payload.len())
                .map_err(|_| Fault::new(Code::OutOfRange, "telemetry payload length exceeds u64"))?;
            bytes = bytes.checked_add(payload_bytes)
                .ok_or_else(|| Fault::new(Code::OutOfRange, "telemetry batch byte overflow"))?;
            if bytes > maximum_bytes {
                return Err(Fault::new(Code::ResourceExhausted, "telemetry batch exceeds limit"));
            }
            if previous_sequence.is_some_and(|previous| envelope.sequence <= previous) {
                return Err(Fault::invalid_argument("telemetry batch sequence is not strictly increasing"));
            }
