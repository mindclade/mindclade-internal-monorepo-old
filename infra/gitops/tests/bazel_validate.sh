...
cert_manager_source="$(resolve_runfile "${MINDCLADE_CERT_MANAGER_SOURCE_RLOCATION:?}")"
...
for required_file in "${archive}" "${cert_manager_source}" "${extractor}" "${tool_marker}" \
  "${schema_marker}" "${toolchain_manifest}"; do
...
export PATH MINDCLADE_KUBERNETES_SCHEMA_DIR
export MINDCLADE_CERT_MANAGER_SOURCE="${cert_manager_source}"
...
exec bash "${staged_repository}/infra/gitops/tests/validate.sh"
