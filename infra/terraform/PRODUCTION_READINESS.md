...
| Signed immutable module publication | BLOCKED | Source now requires protected-main dispatch, post-approval main reauthorization, exact-tag qualification at each mutation boundary, a source-pinned SSH signer fingerprint with fresh evidence, Security environment approval after independent Release tag signing, owner-enforced immutable releases with evidence no older than seven days, exact manifest/checksum/attestation asset digests, and immutable release read-back. The authority contract remains blocked, the environment is not asserted applied, and the connected repository reported immutable releases disabled on 2026-08-23 |
