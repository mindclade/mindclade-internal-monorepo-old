...
| Signed immutable module publication | BLOCKED | Source now requires protected-main dispatch, exact-tag qualification, a source-pinned SSH signer fingerprint with fresh evidence, Security environment approval after independent Release tag signing, manifest/checksum/attestation assets, and immutable release read-back. The authority contract remains blocked, the environment is not asserted applied, and the connected repository reported immutable releases disabled on 2026-08-23 |
