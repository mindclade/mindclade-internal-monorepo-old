...
**Current decision:** Not ready for production apply.<br>
...
|---|---|---|
| Stable module contracts and owners | PASS | Generated interfaces for 32 reusable modules, versioned migration evidence, component ownership, and Platform/Security review routing |
...
- Backendless validation and mock-provider totals must be regenerated after the live-root split.
- Google provider compatibility remains required at both 7.41.0 and 7.45.0.
- TFLint 0.64.0 default Terraform rules remain required for every reusable module.
...
- Terraform interface governance covers reusable modules only; its evidence must be regenerated
  for v0.2.0, including all breaking
...
`github-config` ruleset is deliberately scoped to `mindclade-internal-monorepo` and defaults to `evaluate`;
