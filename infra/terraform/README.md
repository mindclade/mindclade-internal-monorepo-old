...
Reusable modules live in `modules/`. The separately controlled `infrastructure-live` repository
owns deployable roots, provider aliases, remote-state bucket names, tfvars, imports,
and promotion evidence. There are no live-root exceptions in this repository.
