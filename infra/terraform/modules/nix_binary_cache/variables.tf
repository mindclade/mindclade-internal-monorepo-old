...
variable "project_id" {
  description = "Project that owns the Nix binary-cache bucket"
  type        = string
...
variable "data_classification" {
  description = "Governance classification for published binary artifacts; public classification is forbidden"
  type        = string
  default     = "internal"
...
variable "writer_members" {
  description = "Non-public publisher identities granted create-only plus read access"
  type        = set(string)
...
variable "soft_delete_retention_days" {
  description = "Recovery window after a binary-cache object is deleted"
  type        = number
  default     = 30
...
variable "retention_period_seconds" {
  description = "Minimum retention for every published binary-cache object"
  type        = number
  default     = 2592000

  validation {
    condition     = var.retention_period_seconds >= 604800 && var.retention_period_seconds <= 3155760000 && floor(var.retention_period_seconds) == var.retention_period_seconds
    error_message = "retention_period_seconds must be a whole number from 604800 through 3155760000."
  }
}

variable "lock_retention_policy" {
  description = "Permanently lock the retention policy; irreversible and disabled by default"
  type        = bool
  default     = false
}

variable "retention_lock_confirmation" {
  description = "Exact irreversible-action acknowledgement required only when locking retention"
  type        = string
  default     = null
  sensitive   = true
  ...
  validation {
    condition = (
      (!var.lock_retention_policy && var.retention_lock_confirmation == null) ||
      (var.lock_retention_policy && var.retention_lock_confirmation == "LOCKING A CLOUD STORAGE RETENTION POLICY IS IRREVERSIBLE")
    )
    error_message = "Locking retention requires the exact irreversible-action acknowledgement; otherwise leave confirmation null."
  }
}
