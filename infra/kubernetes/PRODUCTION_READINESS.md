...
**Decision:** environment-neutral packages implemented; live activation belongs to `gitops`.
**Last repository review:** 2026-08-20.<br>
...
|---|---|---|
| Environment-neutral package roots render | PASS | Inventoried Kustomize and Helm roots under `base/`, `platform/`, `services/`, and `workloads/` |
...
| Workload activation is absent | PASS | No development, staging, or production overlay exists in this repository |
...
Activation requires one reviewed change in `mindclade/gitops` that names the target
environment and contains all of the following:
...
Environment identity, quota, and workload selection must change in one authoritative
GitOps review. This source repository cannot activate a cluster.
