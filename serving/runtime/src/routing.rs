...
use mindclade_worker_protocol::{AdmissionGrantClaims, DeploymentRoute, RevocationSnapshot, RouteSnapshot};
...
pub fn select_route(request: RouteRequest<'_>) -> FaultResult<DeploymentRoute> {
    ...
    for route in &request.snapshot.claims.routes {
        ...
        if route.lease_expires_unix_millis <= request.now_unix_millis
            ...
            || request.revocations.bundle_revoked(&route.model_bundle.to_string())
            || request.revocations.bundle_revoked(&route.engine_bundle.to_string())
            || route.region != request.grant.region
        {
            continue;
        }
        if let Some(hint) = &request.admission.deployment_hint {
            if hint != &deployment { continue; }
        }
        ...
        if !deployment_allowed && !capabilities_allowed { continue; }
        eligible.push(route);
    }
    if eligible.is_empty() {
        return Err(Fault::new(Code::Unavailable, "no eligible local route exists for this grant"));
    }
    let total_weight = eligible.iter().try_fold(0_u64, |total, route| {
        total.checked_add(u64::from(route.weight))
    ...
    for route in eligible {
        ...
        if slot < weight { return Ok(route.clone()); }
        slot -= weight;
    }
    Err(Fault::internal("weighted route selection exhausted unexpectedly"))
}
