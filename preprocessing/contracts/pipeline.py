...
class PipelinePlan:
    ...
    def validate(self) -> None:
        ...
        if len(ids) != len(self.stages):
            raise ValueError("duplicate preprocessing stage id")
        state = {}
        graph = {s.spec.stage_id: s.dependencies for s in self.stages}

        def visit(i):
            if state.get(i) == 1:
                raise ValueError("preprocessing pipeline contains a cycle")
            if state.get(i) == 2:
                return
            if i not in graph:
                raise ValueError(f"unknown stage dependency {i}")
            state[i] = 1
            for d in graph[i]:
                visit(d)
            state[i] = 2
