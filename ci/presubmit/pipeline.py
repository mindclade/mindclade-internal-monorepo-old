...
def run(command): return subprocess.call(command,cwd=REPO)

def main()->int:
    parser=argparse.ArgumentParser(); parser.add_argument('--static-only',action='store_true'); parser.add_argument('--base'); args=parser.parse_args()
    if run([sys.executable,str(REPO/'tools/analysis/run_architecture_checks.py')]): return 1
    if args.static_only: return 0
    affected=load_affected(); changed=affected.git_changed(args.base)
    if affected.rust_qualification_required(changed):
        if run([sys.executable,str(REPO/'tools/qualification/rust/qualify.py'),'--mode','presubmit']): return 1
    else:
