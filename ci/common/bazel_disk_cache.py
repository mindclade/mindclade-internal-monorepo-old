...
if __package__:
    from . import bazel_remote_cache
else:  # invoked as a script by the workflow, which has no package context
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    import bazel_remote_cache
...
def configure(
    *,
    ...
    # Staged and inert. remote_lines returns "" while activation is disabled, so the file below
    # is byte-identical to what it was before the remote cache was staged. See
    # ci/common/bazel_remote_cache.py for what activation changes and why it is not on yet --
    # including that `--remote_upload_local_results` governs nothing until it is.
    remote = bazel_remote_cache.remote_lines(
        bazel_remote_cache.load_contract(),
        role=role,
        endpoint=os.environ.get("BAZEL_REMOTE_CACHE_URI", ""),
    )
    contents = (
