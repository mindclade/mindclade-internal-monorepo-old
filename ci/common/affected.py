...
GLOBAL_PREFIXES = (
    '.github/', '.buildkite/', 'ci/', 'tools/build/', 'tools/qualification/',
    'MODULE.bazel', 'MODULE.bazel.lock', 'Cargo.toml', 'Cargo.lock',
    'go.mod', 'go.sum', 'flake.nix', 'flake.lock', 'protocols/',
    'architecture/', 'components.toml', 'maturity.toml',
)

RUST_PREFIXES = (
    'libs/rust/', 'protocols/rust/', 'protocols/proto/mindclade/runtime/',
    'services/runtime_gateway/', 'services/runtime_host/', 'services/artifact_proxy/',
    'services/node_agent/', 'services/workers/ingestion/', 'serving/runtime/',
    'Cargo.toml', 'Cargo.lock', 'deny.toml', 'security/rust-supply-chain.toml',
    'tools/qualification/rust/', 'tools/build/nix/', 'flake.nix', 'flake.lock',
)
...
ATTR_RE = re.compile(r'\b(' + '|'.join(DEP_ATTRIBUTES) + r')\s*=\s*(\[[^\]]*\]|[\"\'][^\"\']+[\"\'])', re.S)
...
def select(changed: list[str]) -> list[str]:
    if not changed or any(path == prefix or path.startswith(prefix) for path in changed for prefix in GLOBAL_PREFIXES):
        return ['//...']
    ...
    while queue:
        ...
        for consumer in reverse.get(current, ()):
            if consumer not in seen:
                seen.add(consumer); queue.append(consumer); affected.add(consumer)
            pkg = consumer.split(':', 1)[0]
            if pkg not in seen:
                seen.add(pkg); queue.append(pkg)
