...
def repository_boundary(root: Path) -> list[str]:
    """Enforce the enterprise repository authority split."""

    errors: list[str] = []
    required = (
        Path("BLUEPRINT.md"),
        Path("contracts/repository.yaml"),
        Path(".buildkite/pipeline.yml"),
    )
    forbidden = (
        Path("infra/gitops"),
        Path("infra/terraform/environments"),
        Path("infra/kubernetes/overlays"),
    )
    for path in required:
        if not (root / path).exists():
            errors.append(f"required authority contract is missing: {path}")
    for path in forbidden:
        if (root / path).exists():
            errors.append(f"forbidden live-state authority path exists: {path}")

    environment_names = {"development", "staging", "production"}
    kubernetes_root = root / "infra/kubernetes"
    if kubernetes_root.is_dir():
        for path in kubernetes_root.rglob("overlays"):
            if any((path / environment).exists() for environment in environment_names):
                errors.append(
                    "environment-specific Kubernetes overlay belongs in gitops: "
                    f"{path.relative_to(root)}"
                )

    stale_tokens = ("mindclade-org", "github.com/mindclade/mindclade.git")
    for path in _walk(root):
        if path == root / "tools/dev/validate_repository.py":
            continue
        if path.suffix.lower() not in {
            ...
            ".yaml",
            ".yml",
        } and path.name not in {"CODEOWNERS", "MODULE.bazel"}:
            continue
        text = path.read_text(encoding="utf-8", errors="ignore")
        for token in stale_tokens:
            if token in text:
                errors.append(f"stale repository identity in {path.relative_to(root)}: {token}")
    return errors
