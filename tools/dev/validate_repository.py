...
def structured_files(root: Path) -> list[str]:
    ...
    for p in root.rglob("*.json"):
        ...
        except Exception as exc: errors.append(f"JSON parse failed {p.relative_to(root)}: {exc}")
    for p in root.rglob("*.toml"):
        if any(x in p.parts for x in ("node_modules", ".git")): continue
        try: tomllib.loads(p.read_text())
        except Exception as exc: errors.append(f"TOML parse failed {p.relative_to(root)}: {exc}")
    try:
    ...
    if yaml is not None:
        for pattern in ("*.yaml", "*.yml"):
            for p in root.rglob(pattern):
                if any(x in p.parts for x in ("node_modules", ".git")): continue
                try:
                    for _ in yaml.safe_load_all(p.read_text()): pass
                except Exception as exc: errors.append(f"YAML parse failed {p.relative_to(root)}: {exc}")
    return errors
    ...
    for p in root.rglob("*.md"):
        ...
        for raw in _LINK.findall(text):
            ...
            try: resolved.relative_to(root.resolve())
                ...
                errors.append(f"Markdown link escapes repository: {p.relative_to(root)} -> {raw}"); continue
            if not resolved.exists(): errors.append(f"broken local Markdown link: {p.relative_to(root)} -> {raw}")
    return errors
...
def main() -> int:
    ...
    args=ap.parse_args(); root=args.repo.resolve(); errors=[]

    for name,fn in (("hygiene",hygiene),("structured files",structured_files),("Markdown links",markdown_links)):
        found=fn(root)
        if found: errors.extend(f"{name}: {x}" for x in found)
        else: print(f"PASS {name}")

    rc,out=run([sys.executable,"tools/analysis/run_architecture_checks.py"],root)
    print(out,end="")
    if rc: errors.append("architecture policy failed")

    if args.go != "none":
        rc,out=run(["tools/qualification/go/validate.sh",args.go],root)
        print(out,end="")
        if rc: errors.append(f"Go {args.go} qualification failed")
