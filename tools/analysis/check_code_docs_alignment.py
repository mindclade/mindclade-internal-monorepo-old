...
def _requirements(go_mod: str) -> list[tuple[str, str]]:
    ...
    for raw in go_mod.splitlines():
        line=raw.strip()
        if line == "require (": inside=True; continue
        if inside and line == ")": inside=False; continue
        if inside and line and not line.startswith("//"):
            parts=line.split()
            if len(parts)>=2 and parts[1].startswith("v"): req.append((parts[0],parts[1]))
    return req
...
def check(root: Path) -> list[str]:
    ...
    for rel in REQUIRED_DOCS + REQUIRED_SOURCE:
        if not (root / rel).exists(): errors.append(f"required design/source path missing: {rel}")
    ...
    for token in ("control/runtime_authority", "services/runtime_gateway", "services/runtime_host",
                  "libs/rust/runtime_core", "preprocessing", "control/registry/releases"):
    ...
    for name,service in (("runtime.gateway","runtime_gateway"),("runtime.host","runtime_host")):
        c=by_name.get(name)
        if not c: errors.append(f"component metadata missing {name}"); continue
        if c.get("status") != "implemented":
            errors.append(f"{name}: source core is implemented but components.toml status is {c.get('status')!r}")
        ...
        if "implemented core" not in readme or "production qualification pending" not in readme:
            errors.append(f"services/{service}/README.md must distinguish implemented core from pending qualification")
    ...
    go_mod=(root/"go.mod").read_text(); sum_lines=set((root/"go.sum").read_text().splitlines())
    for mod,ver in _requirements(go_mod):
        prefix=f"{mod} {ver} h1:"
        modprefix=f"{mod} {ver}/go.mod h1:"
        if not any(x.startswith(prefix) for x in sum_lines): errors.append(f"go.sum missing module checksum: {mod} {ver}")
        if not any(x.startswith(modprefix) for x in sum_lines): errors.append(f"go.sum missing go.mod checksum: {mod} {ver}")

    # Docs must never claim source presence implies qualification.
    for rel in ("docs/architecture/system-design-reference.md", "QUALIFICATION.md", "VALIDATION.md"):
...
def main() -> int:
    ...
    root=ap.parse_args().repo.resolve(); errors=check(root)
    for e in errors: print(e)
    print("code/docs alignment check passed" if not errors else f"code/docs alignment check failed: {len(errors)}")
