...
COMPAT = {
    ...
    "mindclade_byte_spec", "mindclade_artifact_manifest", "mindclade_observability",
...
BASELINE = {
    ...
    "checkpoint_io": {"mindclade_artifact_manifest", "mindclade_byte_spec", "mindclade_clock", "mindclade_resource_version"},
...
IMPLEMENTED = [
    "libs/rust/runtime_core", "libs/rust/bytes_io", "libs/rust/manifests",
    "libs/rust/bounded_parse", "libs/rust/bio_formats", "libs/rust/worker_protocol",
    "libs/rust/worker_runtime", "libs/rust/gpu_host", "libs/rust/telemetry",
    "services/runtime_gateway", "services/runtime_host",
]
...
def check(root: Path) -> list[str]:
            ...
            for dep_name, spec in (data.get(section) or {}).items():
                if isinstance(spec, dict) and "path" in spec:
                    ...
                    if not target.exists():
                        errors.append(f"{member}: path dependency missing: {dep_name} -> {spec['path']}")
                        continue
                    target_data = tomllib.loads(target.read_text())
                    actual = target_data.get("package", {}).get("name")
                    expected = spec.get("package", dep_name)
                    if actual != expected:
                        errors.append(f"{member}: path dependency {dep_name} expects package {expected}, found {actual} at {spec['path']}")
    ...
    for cargo in sorted((root / "libs/rust").glob("*/Cargo.toml")):
        ...
        if unexpected:
            errors.append(f"libs/rust/{cargo.parent.name}: new compatibility dependencies forbidden: {sorted(unexpected)}")
    ...
    for rel in IMPLEMENTED:
        ...
        for source in path.rglob("*.rs"):
            ...
            if "SCAFFOLD_" in text:
                errors.append(f"implemented Rust component still contains scaffold constant: {source.relative_to(root)}")
            ...
            if UNSAFE.search(scrubbed):
                errors.append(f"unsafe Rust requires an explicitly audited adapter boundary: {source.relative_to(root)}")
    return errors

def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument("--repo", type=Path, default=Path(__file__).resolve().parents[2]); args = ap.parse_args()
    errors = check(args.repo.resolve())
    for error in errors: print(error)
    print("Rust workspace check passed" if not errors else f"Rust workspace check failed: {len(errors)}")
