...
- materialized and documented all 32 reusable Google Cloud modules without creating
  deployable environment values or cloud state; live roots remain in `infrastructure-live`;
...
- The release workflow references `reusable-oci-build.yml@v1.2.0`. That tag has to be cut in
  `mindclade/.github` before this lands, or the release job fails resolving it.
